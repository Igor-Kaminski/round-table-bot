...
def insert_scoreboard(scoreboard, queue_num):
    ...
    try:
        ...
        if cursor.fetchone():
            print(
                f"Warning: Match with match_id {match_id} already exists. Skipping.")
            return
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?);
            """,
            (match_id, time, region, map_name,
             team1_score, team2_score, queue_num),
        )
        ...
        for idx, player in enumerate(players):
            ...
            cursor.execute(
                "SELECT player_id, discord_id, alt_igns FROM players WHERE player_ign = ?;", (ign,))
            ...
            if not result:
                ...
                cursor.execute(
                    "SELECT player_id, discord_id, alt_igns FROM players;")
                for row in cursor.fetchall():
                    alt_player_id, discord_id, alt_igns_json = row
                    alt_igns = json.loads(
                        alt_igns_json) if alt_igns_json else []
                    if ign in alt_igns:
                ...
                player_id, discord_id, alt_igns_json = result

            cursor.execute(
                """
                INSERT INTO player_stats (
                    match_id, player_id, champ, talent, credits,
                    kills, deaths, assists, damage, taken, objective_time,
                    shielding, healing, self_healing, team
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                """,
                (
...
def link_ign(player_ign, discord_id, force=False):
    """
    Links a discord_id to an IGN. If force is False, only link if IGN and Discord ID are not already linked.
    ...
    try:
        ...
        if ign_result:
            if not force:
                # IGN already linked, do not overwrite
                return False
            # Overwrite existing link for IGN
            cursor.execute(
                "UPDATE players SET discord_id = ? WHERE player_ign = ?;", (discord_id, player_ign))
        elif disc_result:
            # Discord ID exists, update IGN
            cursor.execute(
                "UPDATE players SET player_ign = ? WHERE discord_id = ?;", (player_ign, discord_id))
        else:
            # Neither exists, insert new
            cursor.execute(
...
def update_discord_id(old_discord_id, new_discord_id):
    ...
    try:
        cursor.execute(
            "SELECT player_id FROM players WHERE discord_id = ?;", (
                old_discord_id,)
        )
        result = cursor.fetchone()
        if result:
...
def insert_embed(queue_num, embed_data):
    """
    Inserts embed data into the embeds table, storing only the queue number and the 10 Discord IDs.
    """
    conn = sqlite3.connect("match_data.db")
    cursor = conn.cursor()
    try:
        ...
        cursor.execute(
            "SELECT 1 FROM embeds WHERE queue_num = ?;", (queue_num,))
        if cursor.fetchone():
            print(
...
def read_embeds(queue_num):
    ...
    try:
        cursor.execute(
            "SELECT embed_data FROM embeds WHERE queue_num = ?;", (queue_num,))
        result = cursor.fetchone()
...
def queue_exists(queue_num):
    ...
    try:
        cursor.execute(
            "SELECT 1 FROM matches WHERE queue_num = ?", (queue_num,))
        return cursor.fetchone() is not None
    finally:
        conn.close()
...
def add_alt_ign(discord_id, alt_ign):
    ...
    try:
        ...
        if alt_ign not in alt_igns:
            ...
            cursor.execute("UPDATE players SET alt_igns = ? WHERE player_id = ?;",
            ...
            print(
                f"Alt IGN {alt_ign} already exists for Discord ID {discord_id}.")
            return False
    except sqlite3.Error as e:
        print(f"An error occurred: {e}")
        return False
...
def get_ign_link_info(ign):
    ...
    try:
        cursor.execute(
            "SELECT discord_id FROM players WHERE player_ign = ?;", (ign,))
        result = cursor.fetchone()
        if result:
...
def get_alt_igns(discord_id):
    ...
    try:
        cursor.execute(
            "SELECT alt_igns FROM players WHERE discord_id = ?;", (discord_id,))
        result = cursor.fetchone()
...
def get_stats(player_id):
    ...
    if match_ids:
        cursor.execute(
            f"SELECT match_id, time FROM matches WHERE match_id IN ({','.join(['?']*len(match_ids))})", match_ids)
        match_times = {row[0]: row[1] for row in cursor.fetchall()}
    else:
        match_times = {}
    total_time = sum(match_times.get(mid, 1)
                     for mid in match_ids)
    agg = [sum(col) for col in zip(*[row[:7] for row in rows])]
    if total_time == 0:
        total_time = 1
    norm = [round(val / total_time, 2) for val in agg]
    return {
        "kills": norm[0], "deaths": norm[1], "assists": norm[2],
        "damage": norm[3], "objective_time": norm[4],
        "shielding": norm[5], "healing": norm[6],
        "games": len(rows)
    }
...
def get_top_champs(player_id):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT champ, COUNT(*), SUM(kills), SUM(deaths), SUM(assists), SUM(damage), SUM(objective_time), SUM(shielding), SUM(healing), GROUP_CONCAT(match_id)
        FROM player_stats WHERE player_id = ?
        GROUP BY champ
    ...
    rows = cursor.fetchall()
    champs = []
    for champ, games, kills, deaths, assists, damage, obj_time, shielding, healing, match_ids_str in rows:
        match_ids = [int(mid) for mid in match_ids_str.split(",") if mid]
        if match_ids:
            cursor.execute(
                f"SELECT match_id, time FROM matches WHERE match_id IN ({','.join(['?']*len(match_ids))})", match_ids)
            match_times = {row[0]: row[1] for row in cursor.fetchall()}
            total_time = sum(match_times.get(mid, 1) for mid in match_ids)
        else:
            total_time = games  # fallback, should not happen

        # Get winrate using team column
        cursor.execute("""
            SELECT m.team1_score, m.team2_score, ps.team
            FROM player_stats ps
            JOIN matches m ON ps.match_id = m.match_id
            WHERE ps.player_id = ? AND ps.champ = ?
        """, (player_id, champ))
        match_rows = cursor.fetchall()
        wins = 0
        for t1, t2, team in match_rows:
        ...
        champ_stats = {
            ...
            "games": games,
            "winrate": winrate,
            "kda": f"{round(kills/games, 2)}/{round(deaths/games, 2)}/{round(assists/games, 2)}",
            "damage": round(damage/total_time, 2),
            "objective_time": round(obj_time/total_time, 2),
            "shielding": round(shielding/total_time, 2),
            "healing": round(healing/total_time, 2)
        }
...
def get_winrate_with_against(pid1, pid2):
    ...
    for t1, t2, team in rows:
        if (team == 1 and t1 > t2) or (team == 2 and t2 > t1):
            against_wins += 1
    against_winrate = round(100 * against_wins /
                            against_games, 1) if against_games else 0
...
def compare_players(discord_id1, discord_id2):
    """
    Returns stats for two players:
    - Overall normalized stats (kills, deaths, assists, damage, obj_time, shielding, healing per minute)
    - Top 5 champs with winrate, k/d/a, etc (per minute)
    - Winrate with and against each other
    """
    ...
    stats1 = get_stats(pid1)
    stats2 = get_stats(pid2)
